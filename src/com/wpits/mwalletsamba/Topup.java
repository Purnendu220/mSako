...
public class Topup extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		spinneroprator=(Spinner)findViewById(R.id.spinneroprator);
...
Toast.makeText(getApplicationContext(), "Please enter a valid amount", Toast.LENGTH_LONG).show();					// TODO: handle exception
		...
		next.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				new AsyncTopup().execute();
			}
		});
	}
	public class AsyncTopup extends AsyncTask<String, String, String> {
		ProgressDialog pd;
		String result;
		@Override
		protected void onPreExecute() {

			pd = ProgressDialog.show(Topup.this, "Topup in Progress  ",
					"Please Wait");
		}
...
if(mPin_EditText.getVisibility()==View.VISIBLE)
{
	...
	else if(spinneroprator.getSelectedItemPosition()==0){
		Toast.makeText(getApplicationContext(), "Please select operater of destination mobile", Toast.LENGTH_LONG).show();

	}
	else{
	HttpTransfer httpTransfer = new HttpTransfer();
	String str = httpTransfer.httptransfer(mobile_no, dest_mobile_no, mPin_EditText.getText().toString(), amount, "topup", "Topup", 	String.valueOf(spinneroprator.getSelectedItem()));
    Toast.makeText(getApplicationContext(), str, Toast.LENGTH_LONG).show();
	String otp = "";
	Bundle b = new Bundle();
	b.putString("mobile_no", mobile_no);
	b.putString("otp", otp);
	Intent in = new Intent(Topup.this, AndroidGridLayoutActivity.class);
	in.putExtras(b);
	startActivity(in);
	finish();
	 ...
	 if(spinneroprator.getSelectedItemPosition()==0){
		Toast.makeText(getApplicationContext(), "Please select operater of destination mobile", Toast.LENGTH_LONG).show();

	}
	else{
	Httptopup httpTransfer = new Httptopup();
	String str = httpTransfer.httptopup(mobile_no, dest_mobile_no, mPin, amount, "topup", "Topup", 	String.valueOf(spinneroprator.getSelectedItem()));
    Toast.makeText(getApplicationContext(), str, Toast.LENGTH_LONG).show();
	String otp = "";
	Bundle b = new Bundle();
	b.putString("mobile_no", mobile_no);
