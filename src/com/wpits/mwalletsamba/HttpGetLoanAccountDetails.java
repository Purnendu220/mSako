...
public class HttpGetLoanAccountDetails {
	String responseregcont;
	private Context context;
	String[] splitval ;
	public LoanAccountDetailParser httpuserloanaccount(Context context,String mobileno,String session_id,String loan_acc_id,String usertype)
	{
		...
		try {
			...
			obj.put("req_type", "loanacc");

			obj.put("loan_acc_id", loan_acc_id);

			HttpClient httpclient = new DefaultHttpClient();
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+GlobalIpConfigration.URL_CHAT.toString()); 
			HttpPost httppost = new HttpPost(GlobalIpConfigration.URL_BANK_MSAKO+"user_loan_account_detail");
			...
			{
				parser=new LoanAccountDetailParser(responseregcont);
