...
public class HttpAskmPinAccountDetails {
	String responseregcont;
	private Context context;
	String[] splitval ;
	public UserAccountDetailsParser httpaskmpin(Context context,String mobileno,String mpin,String session_id,String user_type)
	{
		this.context = context;
		//dao=new TodoDAO(context);
		UserAccountDetailsParser parser = null;
		TelephonyManager telephonyManager = (TelephonyManager)context.getSystemService(Context.TELEPHONY_SERVICE);
		telephonyManager.getDeviceId();
		try {
			JSONObject obj=new JSONObject();
			obj.put("type", "AskmPinAccountDetails");
			...
			obj.put("req_type", "acc");

			HttpClient httpclient = new DefaultHttpClient();
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+GlobalIpConfigration.URL_CHAT.toString()); 
			HttpPost httppost = new HttpPost(GlobalIpConfigration.URL_BANK_MSAKO+"user_ask_mpin");
			...
			{
				parser=new UserAccountDetailsParser(responseregcont);
