...
public class HttpGetLoanAccountStatus {
	String responseregcont;
	private Context context;
	String[] splitval ;
	public LoanAccountDetailParser httpuserloanaccount(Context context,String mobileno,String session_id,String usertype)
	{
		...
		try {
			...
			obj.put("req_type", "loanaccstat");


			HttpClient httpclient = new DefaultHttpClient();
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+GlobalIpConfigration.URL_CHAT.toString()); 
			HttpPost httppost = new HttpPost(GlobalIpConfigration.URL_BANK_MSAKO+"user_loan__equest_status");
