...
public class HttpTransferMoney {
	String responseregcont;
	private Context context;
	String[] splitval ;
	public TransferResponseParser httpusertransfer(Context context,String mobileno,String session_id,String dest_mobile,String mpin,String amount,String transfertype,String transferaccount,String usertype)
	{
		this.context = context;
		TransferResponseParser parser = null;
		...
		try {
			...
			obj.put("destination_msisdn", dest_mobile);
			obj.put("mpin", mpin);
			obj.put("amount", amount);
			obj.put("transfertype", transfertype);
			obj.put("tranferaccount", transferaccount);
			obj.put("usertype", usertype);
			HttpClient httpclient = new DefaultHttpClient();
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+GlobalIpConfigration.URL_CHAT.toString()); 
			HttpPost httppost = new HttpPost(GlobalIpConfigration.URL_BANK_MSAKO+"user_moneytransfer");
			...
			{
				parser=new TransferResponseParser(responseregcont);
