...
public class Httppinchange {
	String responseregcont;
	private Context context;
	String[] splitval ;
	public UpdateProfileParser httpchangempin(Context context,String mobileno,String session_id,String old_mpin,String new_mpin,String usertype)
	{
		...
		try {
			...
			obj.put("old_mpin", old_mpin);
			obj.put("new_mpin", new_mpin);

			HttpClient httpclient = new DefaultHttpClient();
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+GlobalIpConfigration.URL_CHAT.toString()); 
			HttpPost httppost = new HttpPost(GlobalIpConfigration.URL_BANK_MSAKO+"user_mpin_change");
